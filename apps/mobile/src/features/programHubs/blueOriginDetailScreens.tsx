...
export function BlueOriginContractDetailScreen({ slug }: { slug: string }) {
  ...
  const canonicalSummary = useMemo(
    () =>
      canonicalContractsPayload?.items.find((entry) => normalizeContractSlug(entry.contractKey) === slug) ?? null,
        ...
        render: () => {
          ...
          return (
            <>
              ...
              <CustomerShellPanel title="Contract summary" description="Program-specific summary from the shared Blue Origin contracts index.">
                <View style={{ gap: 10 }}>
                  <DetailRow
