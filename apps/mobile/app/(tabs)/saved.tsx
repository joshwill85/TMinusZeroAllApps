...
export default function SavedScreen() {
  ...
  async function saveWatchlistRename(watchlist: WatchlistV1) {
    ...
    try {
      ...
      cancelWatchlistRename();
      setNotice({ tone: 'success', message: `Renamed watchlist to "${nextName}".` });
    } catch (error) {
      setNotice({
        tone: 'warning',
  ...
  async function deleteWatchlist(watchlist: WatchlistV1) {
    ...
    try {
      ...
      setNotice({ tone: 'info', message: `Deleted watchlist "${watchlist.name}".` });
    } catch (error) {
      setNotice({
        tone: 'warning',
  ...
  async function savePresetRename(preset: FilterPresetV1) {
    ...
    try {
      ...
      cancelPresetRename();
      setNotice({ tone: 'success', message: `Renamed preset to "${nextName}".` });
    } catch (error) {
      setNotice({
        tone: 'warning',
  ...
  async function setDefaultPreset(preset: FilterPresetV1) {
    ...
    try {
      ...
      setNotice({ tone: 'success', message: `"${preset.name}" is now your default view.` });
    } catch (error) {
      setNotice({
        tone: 'warning',
        message: buildMutationMessage(error, 'Unable to set the default preset.')
      });
    } finally {
      setBusy((current) => ({ ...current, [busyKey]: false }));
    }
  ...
  async function toggleFollowAlertRule(rule: WatchlistRuleV1) {
    ...
    try {
      ...
      setNotice({
        tone: 'warning',
        message: buildMutationMessage(error, existingRuleId ? 'Unable to remove follow alert.' : 'Unable to enable follow alert.')
      });
    } finally {
      setBusy((current) => ({ ...current, [busyKey]: false }));
    }
      ...
      {statusMessage ? (
        <View
          style={{
            borderRadius: 18,
            borderWidth: 1,
            borderColor: statusTone === 'warning' ? 'rgba(251, 191, 36, 0.25)' : theme.stroke,
            backgroundColor: statusTone === 'warning' ? 'rgba(251, 191, 36, 0.08)' : theme.surface,
            paddingHorizontal: 16,
            paddingVertical: 12
          }}
        >
          <Text style={{ color: statusTone === 'warning' ? '#ffd36e' : statusTone === 'success' ? theme.accent : theme.foreground, fontSize: 14, lineHeight: 20 }}>
            {statusMessage}
          </Text>
        </View>
      ) : null}
              ...
              <View style={{ gap: 12 }}>
                {watchlists.map((watchlist, index) => {
                  ...
                  return (
                    <View
                      key={watchlist.id}
                      testID={index === 0 ? 'saved-watchlist-first' : `saved-watchlist-${watchlist.id}`}
                      style={{
                        gap: 12,
                        borderRadius: 18,
                        borderWidth: 1,
                    ...
                    >
                          ...
                          <View style={{ gap: 10 }}>
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                              <InlineActionButton
                                ...
                                disabled={renameBusy || !editingWatchlistName.trim()}
                                tone="accent"
                              />
                              <InlineActionButton label="Cancel" onPress={cancelWatchlistRename} disabled={renameBusy} />
                            </View>
                          </View>
                        ) : (
                          <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                            <View style={{ flex: 1, gap: 4 }}>
                              <Text style={{ color: theme.foreground, fontSize: 16, fontWeight: '700' }}>{watchlist.name}</Text>
                              <Text style={{ color: theme.muted, fontSize: 13, lineHeight: 19 }}>
                                {watchlist.ruleCount} rule{watchlist.ruleCount === 1 ? '' : 's'}
                              </Text>
                            </View>
                            <CustomerShellBadge label={`${watchlist.ruleCount} rule${watchlist.ruleCount === 1 ? '' : 's'}`} />
                          </View>
                        )}

                        {!readOnly && !isEditing ? (
                          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                            <InlineActionButton label="Rename" onPress={() => beginWatchlistRename(watchlist)} disabled={renameBusy || deleteBusy} />
                            <InlineActionButton
                      ...
                      {Object.entries(groupedRules).length === 0 ? (
                        <Text style={{ color: theme.muted, fontSize: 13, lineHeight: 19 }}>No rules in this watchlist yet.</Text>
                      ) : (
                        <View style={{ gap: 12 }}>
                          {Object.entries(groupedRules).map(([groupKey, group]) => (
                            <View key={`${watchlist.id}:${groupKey}`} style={{ gap: 8 }}>
                              <Text style={{ color: theme.muted, fontSize: 11, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>
                                {group.title}
                              </Text>
                              <View style={{ gap: 8 }}>
                                {group.rules.map((rule) => {
                                  const alertType = normalizeWatchlistAlertRuleType(rule.ruleType);
                                  const followAlertRuleId = alertType
                                    ? followAlertRuleIds.get(buildFollowAlertRuleKey(alertType, rule.ruleValue)) ?? null
                                    : null;
                                  const alertBusy = busy[`alert:follow:${rule.id}`];
                                  const removeBusy = busy[`rule:delete:${rule.id}`];
                                  ...
                                  return (
                                    ...
                                    >
                                      <View style={{ gap: 4 }}>
                                        <Text style={{ color: theme.muted, fontSize: 11, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>
                                          {formatWatchlistRuleLabel(rule.ruleType)}
                                        </Text>
                                        <Text style={{ color: theme.foreground, fontSize: 14, fontWeight: '700', lineHeight: 19 }}>
                                          {formatWatchlistRuleValue(rule.ruleType, rule.ruleValue)}
                                        </Text>
                                      </View>
                                      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                                        {!readOnly && canUseAdvancedAlertRules && alertType ? (
                                          <InlineActionButton
                                            label={
                                              alertBusy
                                                ? 'Saving…'
                                            ...
                                            onPress={() => {
                                              void toggleFollowAlertRule(rule);
                                            }}
                                            disabled={alertBusy || alertsLoading}
                                            tone={followAlertRuleId ? 'accent' : 'default'}
                                          />
                                        ) : followAlertRuleId ? (
                                          <InlineActionButton label="Alerts on" onPress={() => undefined} disabled />
                                        ) : null}
                                        {!readOnly ? (
                                          <InlineActionButton
                                            label={removeBusy ? 'Removing…' : 'Remove'}
                                            onPress={() => confirmRemoveRule(watchlist, rule)}
                                            disabled={removeBusy}
                                            tone="danger"
                                          />
                                        ) : null}
                    ...
                    >
                        ...
                        <View style={{ gap: 10 }}>
                          ...
                          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                            <InlineActionButton
                              ...
                              tone="accent"
                            />
                            <InlineActionButton label="Cancel" onPress={cancelPresetRename} disabled={renameBusy} />
                          </View>
                        </View>
                      ) : (
                        <View style={{ gap: 8 }}>
                          <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
                            <Text style={{ color: theme.foreground, fontSize: 16, fontWeight: '700', flex: 1 }}>{preset.name}</Text>
                            {preset.isDefault ? <CustomerShellBadge label="Default" tone="accent" /> : null}
                            <CustomerShellBadge label={`${countPresetFilters(preset.filters)} filter${countPresetFilters(preset.filters) === 1 ? '' : 's'}`} />
                          </View>
                          <Text style={{ color: theme.muted, fontSize: 13, lineHeight: 19 }}>
                      ...
                      {!readOnly && !isEditing ? (
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                          ...
                          />
                          <InlineActionButton label="Rename" onPress={() => beginPresetRename(preset)} disabled={renameBusy || deleteBusy} />
                          <InlineActionButton
                            label={deleteBusy ? 'Deleting…' : 'Delete'}
...
function InlineActionButton({
  ...
  return (
    ...
    >
      <Text style={{ color: colors.textColor, fontSize: 12, fontWeight: '700' }}>{label}</Text>
    </Pressable>
  );
}
...
function groupWatchlistRules(rules: WatchlistRuleV1[]) {
  ...
  for (const [type, title] of groups) {
    const matches = rules.filter((rule) => String(rule.ruleType || '').trim().toLowerCase() === type);
    if (matches.length > 0) {
  ...
  const otherRules = rules.filter((rule) => !knownTypes.has(String(rule.ruleType || '').trim().toLowerCase()));
...
function formatWatchlistRuleLabel(ruleType: string) {
  ...
  return normalized ? normalized.charAt(0).toUpperCase() + normalized.slice(1) : 'Rule';
}
...
function summarizePresetFilters(filters: FilterPresetV1['filters']) {
  ...
  if (typeof record.region === 'string' && record.region.trim()) {
    parts.push(record.region === 'non-us' ? 'Non-US' : record.region === 'all' ? 'All locations' : 'US only');
  }
  ...
  if (typeof record.status === 'string' && record.status.trim() && record.status !== 'all') {
  ...
  return parts.length > 0 ? parts.join(' • ') : 'Saved launch feed filter bundle.';
}

function countPresetFilters(filters: FilterPresetV1['filters']) {
  return Object.entries(filters as Record<string, unknown>).filter(([, value]) => {
