...
export default async function LaunchDetailPage({
  ...
  const heroTitle = resolveLaunchPrimaryTitle(mission.name, launch.name);
  const heroSubtitle = buildLaunchHeroSubtitleLine({
    provider: launch.provider,
    vehicle: rocket.fullName || launch.vehicle,
  ...
  const heroRecoveryChip = launchDetailEnrichment.recovery[0]
    ? normalizeMeaningfulText(
        launchDetailEnrichment.recovery[0].title ||
          launchDetailEnrichment.recovery[0].returnSite ||
  ...
  const heroOverviewItems = [
    {
      label: 'Launch window',
      value: formatDate(launch.net, padTimezone)
    },
    mission.orbit
      ? {
          label: 'Orbit',
    ...
    {
      label: 'Pad',
      value: `${launch.pad.name} (${launch.pad.shortCode || launch.pad.state})`
    },
    heroRecoveryChip
      ? {
          label: 'Recovery',
          value: heroRecoveryChip
        }
      : null
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 overflow-x-hidden px-4 py-10 pb-28 md:overflow-x-visible md:px-8 md:pb-10">
                ...
                <div className="flex flex-wrap gap-2">
                  {primaryWatchUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="rounded-full border border-primary/40 bg-primary/10 px-4 py-2 text-sm font-semibold text-primary transition hover:border-primary"
                    >
                      Watch live
                    </a>
                  ) : null}
                  ...
                  <ShareButton
                    ...
                    title={share.title}
                    text={share.text}
                    className="rounded-full border border-stroke bg-white/[0.04] px-4 py-2 text-sm font-semibold text-text1"
                  />
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full border border-stroke bg-white/[0.04] px-3 py-1 text-xs font-medium text-text2">
                    Visibility / JEP
                  </span>
                  {heroWeatherChip ? (
                    <span className="rounded-full border border-stroke bg-white/[0.04] px-3 py-1 text-xs font-medium text-text2">
                      Weather: {heroWeatherChip}
                    </span>
                  ) : null}
                  {heroRecoveryChip ? (
                    <span className="rounded-full border border-stroke bg-white/[0.04] px-3 py-1 text-xs font-medium text-text2">
                      Recovery: {heroRecoveryChip}
                    </span>
                  ) : null}
                </div>
                {heroNextEvent ? (
      ...
      <section id="vehicle" className="space-y-4">
