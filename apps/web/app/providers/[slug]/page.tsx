...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const title = `${provider.name} news | ${SITE_META.siteName}`;
  const description = `Latest Spaceflight News API coverage linked to ${provider.name} launches.`;
...
export default async function ProviderNewsPage({ params }: { params: { slug: string } }) {
  const provider = await fetchProviderBySlug(params.slug);
  if (!provider) return notFound();

  const supabaseReady = isSupabaseConfigured();
  const data = supabaseReady ? await fetchProviderNews(provider.name) : emptyProviderNews();
  const providerLogo = normalizeImageUrl(provider.logoUrl || provider.imageUrl || null);
  const providerMeta = [provider.type, provider.countryCode].filter(Boolean).join(' - ');
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-10 md:px-8">
      <header className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        <div className="space-y-4">
          <div>
            <p className="text-xs uppercase tracking-[0.14em] text-text3">Provider</p>
            <h1 className="text-3xl font-semibold text-text1">{provider.name}</h1>
          </div>
          <p className="max-w-2xl text-sm text-text2">
            Latest coverage tied to {provider.name} launches from Spaceflight News API.
          </p>
          {providerMeta && <div className="text-xs uppercase tracking-[0.14em] text-text3">{providerMeta}</div>}
          {provider.description && <p className="max-w-2xl text-sm text-text2">{provider.description}</p>}
      ...
      <section className="mt-6 rounded-2xl border border-stroke bg-surface-1 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-xs uppercase tracking-[0.1em] text-text3">Coverage</div>
            <h2 className="text-xl font-semibold text-text1">Latest articles</h2>
          </div>
          ...
          <>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {items.map((item) => {
                ...
                const published = formatNewsDate(item.published_at ?? item.updated_at);
                const authors = formatAuthors(item.authors);
                const badge = formatNewsType(item.item_type);
                const site = item.news_site || 'Spaceflight News';
                const launchLink = buildLaunchLink(data.launchesByItem[item.snapi_uid] || [], data.launchById);
                      ...
                      <div className="mt-auto flex flex-wrap items-center gap-2 text-[11px] text-text3">
                        ...
                        {launchLink && (
                          <>
                            ...
                            <Link href={buildLaunchHref({ id: launchLink.id, name: launchLink.name || 'Launch' })} className="hover:text-text1">
                              {launchLink.name}
                            </Link>
                            {launchLink.dateLabel && <span className="text-text4">({launchLink.dateLabel})</span>}
                            {launchLink.extraCount > 0 && <span className="text-text4">+{launchLink.extraCount} more</span>}
                          </>
                        )}
                      </div>
                    </div>
...
const fetchProviderNews = cache(async (providerName: string): Promise<ProviderNewsData> => {
  ...
  for (const chunk of chunkArray([...snapiUids], CHUNK_SIZE)) {
    const { data, error } = await supabase
      .from('snapi_items')
      .select('snapi_uid, item_type, title, url, news_site, summary, image_url, published_at, updated_at, authors, featured')
      .in('snapi_uid', chunk);
    if (error || !data) continue;
    for (const row of data) {
  ...
  const sorted = Array.from(itemsById.values()).sort((a, b) => getNewsSortMs(b) - getNewsSortMs(a));
  return {
...
function buildLaunchLink(launchIds: string[], launchById: Record<string, LaunchSummary>) {
