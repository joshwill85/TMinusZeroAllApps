...
import {
  fetchSatelliteOwnerIndexBatch,
  fetchSatellitePreviewBatch
} from '@/lib/server/satellites';
...
export function generateMetadata(): Metadata {
  ...
  const description =
    'Browse searchable NORAD satellite records, owner hubs, and launch associations.';
...
export default async function SatellitesIndexPage() {
  const [satellites, ownerRows] = await Promise.all([
    fetchSatellitePreviewBatch(60, 0),
    fetchSatelliteOwnerIndexBatch(24, 0)
  ]);
  const siteUrl = getSiteUrl().replace(/\/+$/, '');
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 md:px-8">
      ...
      <header className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-text2">
          Browse recently updated NORAD catalog objects and jump into
          owner-level indexes for satellites tied to specific operators or
          countries.
        </p>
        ...
        {satellites.length ? (
          <ul className="mt-4 grid gap-3 md:grid-cols-2">
            {satellites.map((sat) => {
              ...
              const ownerHref = sat.owner
                ? buildSatelliteOwnerHref(sat.owner)
                : null;
              const ownerLabel = sat.owner
                ? formatSatelliteOwnerLabel(sat.owner) || sat.owner
                : null;
              return (
                <li
                  key={sat.noradCatId}
          ...
          <p className="mt-3 text-sm text-text3">
            Satellite preview data is not available yet.
          </p>
        )}
      </section>
