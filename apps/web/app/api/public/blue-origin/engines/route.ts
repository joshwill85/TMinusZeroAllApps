...
export async function GET(request: Request) {
  ...
  try {
    const payload = await fetchBlueOriginEngines(mission);
    ...
    console.error('blue origin engines api error', error);
    return NextResponse.json({ error: 'engines_failed' }, { status: 500 });
  }
}
