...
export async function GET(request: Request) {
  ...
  const sourceClass = parseArtemisSourceClassFilter(searchParams.get('sourceClass'));
  if (!sourceClass) return NextResponse.json({ error: 'invalid_source_class' }, { status: 400 });
