...
import { fetchArtemisContractNoticesByPiid } from '@/lib/server/artemisContracts';
...
export async function GET(request: Request, context: { params: { piid: string } }) {
  ...
  const limit = parseLimit(url.searchParams.get('limit'), 250, 1, 2000);
  ...
  const notices = await fetchArtemisContractNoticesByPiid(piid, { limit });
