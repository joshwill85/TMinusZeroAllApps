...
import { BillingStripeRouteError, resumeStripeSubscription } from '@/lib/server/billingStripe';
...
export async function POST(request: Request) {
  try {
    ...
    const payload = await resumeStripeSubscription(session);
    ...
    console.error('billing resume route error', error);
