...
import {
  fetchArtemisMissionComponents,
  fetchArtemisPeople
} from '@/lib/server/artemisMissionSections';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description =
    'Artemis II (Artemis 2) launch date, countdown, mission updates, crew details, and watch links.';
  const images = [
    {
...
export default async function ArtemisIIMissionPage() {
  ...
  const collectionPageJsonLd = {
    ...
    description:
      'Artemis II (Artemis 2) mission schedule, countdown, crew overview, and recent updates.',
  ...
  const eventJsonLd =
    nextLaunch != null
      ? {
          ...
          organizer: nextLaunch.provider
            ? { '@type': 'Organization', name: nextLaunch.provider }
            : undefined,
  ...
  const itemListJsonLd =
    mission.upcoming.length > 0
      ? {
          ...
          itemListElement: mission.upcoming
            .slice(0, 25)
            .map((launch, index) => ({
              '@type': 'ListItem',
              position: index + 1,
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 md:px-8">
      ...
      <header className="space-y-4">
        ...
        <p className="max-w-3xl text-sm text-text2">
          Mission-focused coverage for Artemis II including launch timing,
          countdown visibility, crew notes, and schedule changes. For broader
          context, visit the{' '}
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-text3">
          ...
          <span className="rounded-full border border-stroke px-3 py-1">
            Data source: Launch cache + SNAPI{' '}
            {mission.coverage.hasSocial ? '+ social posts' : ''}
          </span>
          <span className="rounded-full border border-stroke px-3 py-1">
      ...
      <section className="rounded-2xl border border-stroke bg-surface-1 p-4">
        ...
        <p className="mt-2 text-sm text-text2">
          Artemis II is the first planned crewed mission in NASA&apos;s Artemis
          campaign and is commonly searched as both Artemis II and Artemis 2.
          This page tracks schedule changes, timing, and mission readiness
          signals as the launch window evolves.
        </p>
      </section>
        ...
        {nextLaunch ? (
          <div className="mt-3 space-y-3 rounded-xl border border-stroke bg-surface-0 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <Link
                  href={buildLaunchHref(nextLaunch)}
            ...
            <TimeDisplay
              net={nextLaunch.net}
              netPrecision={nextLaunch.netPrecision}
              fallbackTimeZone={nextLaunch.pad.timezone}
            />
          </div>
        ) : (
          <p className="mt-3 text-sm text-text2">
            No Artemis II launch window is currently available in the feed. This
            page stays updated as timing data changes.
          </p>
        )}
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-stroke bg-surface-1 p-4">
          ...
          {mission.crewHighlights.length ? (
            ...
            <p className="mt-3 text-sm text-text2">
              Crew details are not currently present in the mission feed
              payload.
            </p>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-stroke bg-surface-1 p-4">
          ...
          {watchLinks.length ? (
            ...
            <p className="mt-3 text-sm text-text2">
              No public stream URLs are listed yet. Watch links appear
              automatically when sources publish them.
            </p>
          )}
        </div>
      </section>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="rounded-full border border-stroke px-3 py-1 text-xs uppercase tracking-[0.08em] text-text3">
            {crewProfiles.length} profiles
          </span>
        </div>
        <p className="mt-1 text-xs text-text3">
          Official profiles are sourced from NASA and partner agency biography
          pages.
        </p>
        {crewProfiles.length ? (
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {crewProfiles.map((person) => (
              <div
                key={person.id}
                className="flex gap-3 rounded-xl border border-stroke bg-surface-0 p-3"
              >
                {person.portraitUrl ? (
                  <Image
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {person.summary ? (
                    <p className="mt-2 text-xs text-text2">
                      {truncateText(person.summary, 240)}
                    </p>
                  ) : null}
                </div>
              </div>
          ...
          <p className="mt-3 text-sm text-text2">
            No official crew biographies are currently available.
          </p>
        )}
      </section>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="rounded-full border border-stroke px-3 py-1 text-xs uppercase tracking-[0.08em] text-text3">
            {missionComponents.length} items
          </span>
        </div>
        <p className="mt-1 text-xs text-text3">
        ...
        {missionComponents.length ? (
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {missionComponents.map((component) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-text2">
                  {truncateText(component.description, 260)}
                </p>
          ...
          <p className="mt-3 text-sm text-text2">
            No Artemis II mission components are currently available.
          </p>
        )}
      </section>
      ...
      <LaunchList
        title="Upcoming Artemis II launches"
        launches={mission.upcoming}
        emptyLabel="No upcoming Artemis II launches in the feed yet."
      />
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="text-xs text-text3">
                    {formatUpdatedLabel(change.date)}
                  </span>
                </div>
                ...
                {change.href ? (
                  <Link
                    href={change.href}
                    className="mt-2 inline-block text-xs uppercase tracking-[0.1em] text-primary hover:text-primary/80"
                  >
                    Open launch detail
                  </Link>
                ) : null}
