...
import { fetchBlueOriginFlights, getBlueOriginMissionLabel } from '@/lib/server/blueOriginEntities';
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 md:px-8">
      ...
      <header className="space-y-3">
        ...
        <h1 className="text-3xl font-semibold text-text1">Blue Origin Flights</h1>
        <p className="max-w-3xl text-sm text-text2">
          High-level flight index grouped by mission family, with canonical launch-detail links.
        </p>
        <div className="flex flex-wrap items-center gap-2 text-xs text-text3">
          <span className="rounded-full border border-stroke px-3 py-1">Flights tracked: {response.items.length}</span>
        </div>
      </header>
            ...
            <section key={group.missionKey} className="rounded-2xl border border-stroke bg-surface-1 p-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <BlueOriginRouteTraceLink
                  href={group.missionKey === 'blue-origin-program' ? '/blue-origin' : `/blue-origin/missions/${group.missionKey}`}
                    ...
                    <li key={flight.id} className="rounded-xl border border-stroke bg-surface-0 p-3">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-text3">
                        ...
                        {flight.officialMissionUrl ? (
                          <a href={flight.officialMissionUrl} target="_blank" rel="noreferrer" className="text-primary hover:text-primary/80">
                            Official
                          </a>
                        ) : null}
                      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-text3">
        ...
        <Link href="/blue-origin/travelers" className="rounded-full border border-stroke px-3 py-1 uppercase tracking-[0.14em] hover:text-text1">
          Crew
        </Link>
...
function groupByMission(items: Awaited<ReturnType<typeof fetchBlueOriginFlights>>['items']) {
  ...
  return [...byMission.entries()]
    .map(([missionKey, missionItems]) => ({
      missionKey,
      missionLabel: getBlueOriginMissionLabel(missionKey as Parameters<typeof getBlueOriginMissionLabel>[0]),
      items: missionItems.sort((a, b) => {
        const left = Date.parse(a.launchDate || '');
        const right = Date.parse(b.launchDate || '');
        if (!Number.isFinite(left) && !Number.isFinite(right)) return a.flightCode.localeCompare(b.flightCode);
