...
import {
  ChronoHelixTimeline,
  type TimelineNode
} from '@/components/ChronoHelixTimeline';
...
export default async function BlueOriginVehicleDetailPage({
  params
}: {
  params: Params;
}) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: 'Vehicles',
        item: `${siteUrl}/blue-origin/vehicles`
      },
      {
        '@type': 'ListItem',
        position: 4,
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 md:px-8">
      ...
      <header className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-text2">
          {vehicle.description ||
            `${vehicle.displayName} mission profile, linked engines, and mission-specific flight records.`}
        </p>
        <div className="flex flex-wrap items-center gap-2 text-xs text-text3">
          <span className="rounded-full border border-stroke px-3 py-1">
            Mission: {getBlueOriginMissionLabel(vehicle.missionKey)}
          </span>
          <span className="rounded-full border border-stroke px-3 py-1">
            Status: {vehicle.status || 'TBD'}
          </span>
          <span className="rounded-full border border-stroke px-3 py-1">
            First flight: {vehicle.firstFlight || 'TBD'}
          </span>
          <span className="rounded-full border border-stroke px-3 py-1">
      ...
      <section className="rounded-2xl border border-stroke bg-surface-1 p-4">
        ...
        <p className="mt-2 text-sm text-text2">
          This vehicle maps to the{' '}
          {getBlueOriginMissionLabel(vehicle.missionKey)} mission route.
        </p>
        <div className="mt-3">
          <BlueOriginRouteTraceLink
            href={
              vehicle.missionKey === 'blue-origin-program'
                ? '/blue-origin'
                : `/blue-origin/missions/${vehicle.missionKey}`
            }
      ...
      <section className="rounded-2xl border border-stroke bg-surface-1 p-4">
        ...
        {engines.length ? (
            ...
            {engines.map((binding) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <Link
                      href={`/blue-origin/engines/${binding.engineSlug}`}
                      className="text-sm font-semibold text-text1 hover:text-primary"
                    >
                      {binding.engine?.displayName ||
                        binding.engineSlug.toUpperCase()}
                    </Link>
                  ...
                  <span className="text-xs text-text3">
                    {binding.engine?.status || 'Status TBD'}
                  </span>
                </div>
                {binding.notes ? (
          ...
          <p className="mt-3 text-sm text-text3">
            No engine links are available yet.
          </p>
        )}
      </section>

      <ChronoHelixTimeline
        nodes={buildBlueOriginVehicleTimelineNodes(
          flights.items,
          vehicle.displayName
        )}
