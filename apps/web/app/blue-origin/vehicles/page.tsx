    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 md:px-8">
      ...
      <header className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-text2">
          Program-level index of Blue Origin vehicles with dedicated pages,
          mission routing, and linked engine coverage.
        </p>
        <div className="flex flex-wrap items-center gap-2 text-xs text-text3">
          <span className="rounded-full border border-stroke px-3 py-1">
            Vehicles tracked: {response.items.length}
          </span>
          <span className="rounded-full border border-stroke px-3 py-1">
      ...
      <section className="rounded-2xl border border-stroke bg-surface-1 p-4">
        {response.items.length ? (
            ...
            {response.items.map((vehicle) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <Link
                      href={`/blue-origin/vehicles/${vehicle.vehicleSlug}`}
                      className="text-base font-semibold text-text1 hover:text-primary"
                    >
                      {vehicle.displayName}
                    </Link>
                    <p className="mt-1 text-xs text-text3">
                      {getBlueOriginMissionLabel(vehicle.missionKey)}
                    </p>
                  </div>
          ...
          <p className="text-sm text-text3">
            No vehicle records are available yet.
          </p>
        )}
      </section>
