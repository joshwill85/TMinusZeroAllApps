  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-10 md:px-8">
      ...
      <header className="space-y-3">
        <p className="text-xs uppercase tracking-[0.14em] text-text3">Crew Directory</p>
        <h1 className="text-3xl font-semibold text-text1">Blue Origin Crew</h1>
        <p className="max-w-3xl text-sm text-text2">
          Canonical directory of Blue Origin crew profiles, with New Shepard flight references and mission-linked profile pages.
        </p>
        <div className="flex flex-wrap items-center gap-2 text-xs text-text3">
          <span className="rounded-full border border-stroke px-3 py-1">Crew tracked: {response.items.length}</span>
        </div>
      </header>
      ...
      {response.items.length ? (
        <section className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {response.items.map((item) => (
            <article key={item.travelerSlug} className="rounded-xl border border-stroke bg-surface-1 p-3">
              <div className="flex items-start gap-3">
                {item.imageUrl ? (
                  ...
                  <span className="h-12 w-12 rounded-md border border-stroke bg-surface-2/40" aria-hidden="true" />
                )}
                <div className="min-w-0">
                  <Link href={`/blue-origin/travelers/${item.travelerSlug}`} className="text-sm font-semibold text-text1 hover:text-primary">
                    {item.name}
                  </Link>
                  <p className="mt-1 text-xs text-text3">
                    {[item.roles[0] || null, item.nationalities[0] || null].filter(Boolean).join(' • ') || 'Crew'}
                  </p>
                </div>
              </div>

              <div className="mt-3 flex flex-wrap items-center gap-1 text-[11px] text-text3">
                <span className="rounded-full border border-stroke px-2 py-0.5">Flights: {item.flightCount}</span>
                <span className="rounded-full border border-stroke px-2 py-0.5">Launches: {item.launchCount}</span>
                <span className="rounded-full border border-stroke px-2 py-0.5">Confidence: {item.confidence}</span>
              </div>

              <div className="mt-3 text-xs text-text3">
                <p>
                  Latest flight: {item.latestFlightCode ? item.latestFlightCode.toUpperCase() : 'Pending'}
                  {item.latestLaunchDate ? ` • ${formatDate(item.latestLaunchDate)}` : ''}
                </p>
                {item.latestLaunchHref ? (
                  <Link href={item.latestLaunchHref} className="mt-1 inline-block text-primary hover:text-primary/80">
                    Open latest mission
                  </Link>
                ) : null}
              </div>
        ...
        <section className="rounded-2xl border border-stroke bg-surface-1 p-4">
          <p className="text-sm text-text3">No crew profiles are currently available.</p>
        </section>
      )}
