    ...
    () =>
      appendClaimToken(
        buildAuthHref('sign-in', {
          returnTo,
          intent: claimToken ? 'upgrade' : authIntent
        }),
        claimToken
      ),
      ...
      .catch((error: unknown) => {
        ...
        setClaimError(
          error instanceof Error
            ? error.message
            : 'Unable to validate this Premium claim.'
        );
      })
      .finally(() => {
        if (!cancelled) {
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-text3">
          Account creation now happens only after a verified Premium purchase.
          Accounts without Premium stay on the public tier.
        </p>
      </div>
      ...
      {!claimToken ? (
        <div className="space-y-3 rounded-2xl border border-stroke bg-surface-1 p-4 text-sm text-text2">
          <p>
            Standalone public sign-up has been removed. Upgrade to Premium
            first, then create an account to claim it.
          </p>
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Link
              href={buildAuthHref('sign-in', { returnTo, intent: authIntent })}
      ...
      {claimToken && claim?.status === 'pending' ? (
        <div className="rounded-2xl border border-stroke bg-surface-1 p-4 text-sm text-text2">
          Your Premium purchase is still being verified. Return to Upgrade and
      ...
      {claimToken && claim?.status === 'claimed' ? (
        <div className="rounded-2xl border border-stroke bg-surface-1 p-4 text-sm text-text2">
          This Premium purchase is already attached to an account. Sign in to
          manage it.
        </div>
      ) : null}
      ...
      {claimToken && claim?.status === 'verified' ? (
        <>
          <div className="rounded-2xl border border-primary/30 bg-[rgba(34,211,238,0.08)] px-4 py-3 text-sm text-text2">
            Premium is verified. Create an account to claim it now, or sign in
