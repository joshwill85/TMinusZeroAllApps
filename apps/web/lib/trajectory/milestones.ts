...
export function applyTrajectoryMilestoneProjection({
  ...
  for (const window of trackWindows) {
    windows.set(window.trackKind, window);
  }
  ...
  const projected = milestones.map((milestone) => {
    ...
    const window = windows.get(next.trackKind);
    if (!window) {
      next.projectable = false;
    ...
    if (next.tPlusSec < window.minTPlusSec || next.tPlusSec > window.maxTPlusSec) {
      next.projectable = false;
