...
serve(async (req) => {
  ...
  const { runId } = await startIngestionRun(supabase, 'celestrak_gp_ingest');
  ...
  try {
    const settings = await getSettings(supabase, ['celestrak_gp_job_enabled', 'celestrak_gp_max_datasets_per_run']);
    const enabled = readBooleanSetting(settings.celestrak_gp_job_enabled, true);
    if (!enabled) {
    ...
    const maxDatasets = clampInt(
      readNumberSetting(settings.celestrak_gp_max_datasets_per_run, DEFAULTS.maxDatasetsPerRun),
      1,
      25
    );
    ...
    for (const dataset of datasets) {
      ...
      try {
        ...
        stats.membershipsUpserted = (stats.membershipsUpserted as number) + result.membershipsUpserted;
...
async function ingestGpGroup({
  ...
  const res = await fetchJsonWithRetries<any[]>(url, { headers: { 'User-Agent': USER_AGENT, accept: 'application/json' } }, { retries: 3, backoffMs: 1000 });
  if (!res.ok) {
  ...
  await upsertInChunks(supabase, 'satellite_group_memberships', membershipRows, {
