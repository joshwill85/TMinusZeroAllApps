...
serve(async (req) => {
  ...
  const { runId } = await startIngestionRun(supabase, 'll2_payload_backfill_page');
  ...
  try {
    ...
    const jobEnabled = readBooleanSetting(settings.ll2_payload_backfill_job_enabled, false);
    if (!jobEnabled && !force) {
    ...
    const lastSuccessAt = readStringSetting(settings.ll2_payload_backfill_last_success_at, '');
    const lastSuccessMs = Date.parse(lastSuccessAt);
    const lastError = readStringSetting(settings.ll2_payload_backfill_last_error, '').trim();
    ...
    async function touchSuccess() {
      ...
      await upsertSettings(supabase, [
        { key: 'll2_payload_backfill_last_success_at', value: nowIso },
        { key: 'll2_payload_backfill_last_error', value: '' }
      ]);
    }

    const limit = clampInt(readNumberSetting(settings.ll2_payload_backfill_limit, DEFAULTS.backfillLimit), 1, 100);
    ...
    const cursor = await ensureCursor(supabase, readStringSetting(settings.ll2_payload_backfill_cursor, BACKFILL_EPOCH));
    const offset = await ensureOffset(supabase, settings.ll2_payload_backfill_offset);
    ...
    if (!launches.length) {
      await upsertSettings(supabase, [
        { key: 'll2_payload_backfill_job_enabled', value: false },
        { key: 'll2_payload_backfill_done', value: true },
        { key: 'll2_payload_backfill_completed_at', value: nowIso }
      ]);
      await touchSuccess();
    ...
    let payloadFlightCount = 0;
    const payloadIds = new Set<number>();
    const payloadLandingIds = new Set<number>();
    let payloadDockingEventCount = 0;
    if (!spacecraftOnly) {
      payloadFlightCount = launches.reduce((sum: number, row: any) => {
        const flights = Array.isArray(row?.rocket?.payloads) ? row.rocket.payloads : [];
        return sum + flights.length;
      }, 0);
      for (const row of launches as any[]) {
        const flights = Array.isArray(row?.rocket?.payloads) ? row.rocket.payloads : [];
        for (const pf of flights) {
          const payloadId = typeof pf?.payload?.id === 'number' ? pf.payload.id : null;
          if (payloadId != null) payloadIds.add(payloadId);
          const landingId = typeof pf?.landing?.id === 'number' ? pf.landing.id : null;
          if (landingId != null) payloadLandingIds.add(landingId);
          const dockings = Array.isArray(pf?.docking_events) ? pf.docking_events : [];
          payloadDockingEventCount += dockings.length;
        }
      }
    }

    const spacecraftFlightCount = launches.reduce((sum: number, row: any) => {
      const flights = Array.isArray(row?.rocket?.spacecraft_stage) ? row.rocket.spacecraft_stage : [];
      return sum + flights.length;
    }, 0);
    const spacecraftIds = new Set<number>();
    const spacecraftLandingIds = new Set<number>();
    let spacecraftDockingEventCount = 0;
    for (const row of launches as any[]) {
      const flights = Array.isArray(row?.rocket?.spacecraft_stage) ? row.rocket.spacecraft_stage : [];
      for (const sf of flights) {
        const spacecraftId = typeof sf?.spacecraft?.id === 'number' ? sf.spacecraft.id : null;
        if (spacecraftId != null) spacecraftIds.add(spacecraftId);
        const landingId = typeof sf?.landing?.id === 'number' ? sf.landing.id : null;
        if (landingId != null) spacecraftLandingIds.add(landingId);
        const dockings = Array.isArray(sf?.docking_events) ? sf.docking_events : [];
        spacecraftDockingEventCount += dockings.length;
      }
    }
    ...
    if (state.cursor !== cursor) {
      await upsertSettings(supabase, [
        { key: 'll2_payload_backfill_cursor', value: state.cursor },
        { key: 'll2_payload_backfill_offset', value: state.offset }
      ]);
    } else if (state.offset !== offset) {
      await upsertSetting(supabase, 'll2_payload_backfill_offset', state.offset);
    }
    ...
    const result = {
      ...
      upserted: launchUpserts,
      payloadFlights: spacecraftOnly ? 0 : payloadFlightCount,
      payloads: spacecraftOnly ? 0 : payloadIds.size,
      landings: spacecraftOnly ? 0 : payloadLandingIds.size,
      dockingEvents: spacecraftOnly ? 0 : payloadDockingEventCount,
      spacecraftFlights: spacecraftFlightCount,
      spacecrafts: spacecraftIds.size,
      spacecraftLandings: spacecraftLandingIds.size,
      spacecraftDockingEvents: spacecraftDockingEventCount,
      total,
      cursorStart: cursor,
    ...
    try {
      await upsertSetting(supabase, 'll2_payload_backfill_last_error', message);
    } catch (inner) {
      console.error('Failed to persist ll2_payload_backfill_last_error', stringifyError(inner));
    }
...
async function ensureCursor(supabase: ReturnType<typeof createSupabaseAdminClient>, cursor: string) {
  ...
  await upsertSetting(supabase, 'll2_payload_backfill_cursor', BACKFILL_EPOCH);
...
async function ensureOffset(supabase: ReturnType<typeof createSupabaseAdminClient>, value: unknown) {
  ...
  if (shouldPersist) {
    await upsertSetting(supabase, 'll2_payload_backfill_offset', offset);
  }
