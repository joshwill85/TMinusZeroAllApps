...
type SamQueryPolicy = {
  ...
  invalidRequestCooldownDays: number;
...
type SamEndpointBacklogSignals = {
  contractAwardsCandidates: number;
  opportunitiesCandidates: number;
};

type SamRequestAllocationPlan = {
  contractAwardsRequests: number;
  opportunitiesRequests: number;
...
const SETTING_SAM_QUERY_COOLDOWN_DAYS_INVALID_REQUEST = 'artemis_sam_query_cooldown_days_invalid_request';
...
const SETTING_SAM_REQUEST_ALLOCATION_ENABLED = 'artemis_sam_request_allocation_enabled';
const SETTING_SAM_PROBE_MAX_BUDGET_SHARE = 'artemis_sam_probe_max_budget_share';
const SETTING_SAM_PROBE_MIN_POST_BUDGET = 'artemis_sam_probe_min_post_budget';
const SETTING_SAM_OPPORTUNITIES_API_WEIGHT_WHEN_DATA_SERVICES =
...
const DEFAULT_SAM_QUERY_COOLDOWN_DAYS_INVALID_REQUEST = 7;
...
const DEFAULT_SAM_REQUEST_ALLOCATION_ENABLED = true;
const DEFAULT_SAM_PROBE_MAX_BUDGET_SHARE = 0.4;
const DEFAULT_SAM_PROBE_MIN_POST_BUDGET = 2;
const DEFAULT_SAM_OPPORTUNITIES_API_WEIGHT_WHEN_DATA_SERVICES = 0.6;
const DEFAULT_SAM_ENTITY_SYNC_ENABLED = false;
const DEFAULT_SAM_ENTITY_API_URL = 'https://api.sam.gov/entity-information/v4/entities';
const DEFAULT_SAM_CONTRACT_AWARDS_INCLUDE_DELETED = false;
const DEFAULT_SAM_CONTRACT_AWARDS_INCLUDE_SECTIONS = 'coreData,contractId,nasaSpecific';
const DEFAULT_SAM_CONTRACT_AWARDS_EXTRACT_ENABLED = false;
...
const SAM_CONTRACT_AWARDS_ACTION_SCAN_LIMIT_MAX = 10_000;
...
const STALE_INGESTION_RUN_TIMEOUT_MS = 45 * 60 * 1000;
const STALE_INGESTION_RUN_REPAIR_LIMIT = 100;
...
const SAM_CONTRACT_AWARDS_REQUESTS_PER_RUN_LIMIT = 5;
const SAM_OPPORTUNITIES_REQUESTS_PER_RUN_LIMIT = 5;
const SAM_OPPORTUNITIES_DATA_SERVICES_REQUESTS_PER_RUN_LIMIT = 2;
const SAM_CONTRACT_AWARDS_ALLOWED_QUERY_PARAMS = [
  'api_key',
  'piid',
  'referencedIdvPiid',
  'piidSubtierCode',
...
const SAM_CONTRACT_AWARDS_EXTRACT_ALLOWED_QUERY_PARAMS = [
  ...SAM_CONTRACT_AWARDS_ALLOWED_QUERY_PARAMS,
  'format'
] as const;
const SAM_OPPORTUNITIES_ALLOWED_QUERY_PARAMS = [
  'api_key',
  'solnum',
  'q',
  'organizationName',
...
function rowsPerRequest(rows: number, requests: number) {
  if (!Number.isFinite(rows) || !Number.isFinite(requests) || requests <= 0) return 0;
  return Math.max(0, rows / requests);
}
...
function planSamRequestAllocation(input: {
  totalRequests: number;
  runContractAwards: boolean;
  runOpportunities: boolean;
  singlePassPerEndpoint: boolean;
  allocationEnabled: boolean;
  opportunitiesDataServicesEnabled: boolean;
  opportunitiesApiDeltaOnly: boolean;
  opportunitiesApiWeightWhenDataServices: number;
  contractAwardsBacklog: number;
  opportunitiesBacklog: number;
  contractAwardsYield: number;
  opportunitiesYield: number;
}): SamRequestAllocationPlan {
  const totalRequests = Math.max(0, Math.trunc(input.totalRequests));
  if (totalRequests < 1) {
    return {
      contractAwardsRequests: 0,
      opportunitiesRequests: 0,
      rationale: { reason: 'no_request_budget' }
    };
  }

  const contractEnabled = Boolean(input.runContractAwards);
  const opportunitiesEnabled = Boolean(input.runOpportunities);
  if (contractEnabled && !opportunitiesEnabled) {
    return {
      contractAwardsRequests: totalRequests,
      opportunitiesRequests: 0,
      rationale: { reason: 'contract_awards_only' }
    };
  }
  if (!contractEnabled && opportunitiesEnabled) {
    return {
      contractAwardsRequests: 0,
      opportunitiesRequests: totalRequests,
      rationale: { reason: 'opportunities_only' }
    };
  }
  ...
  if (!contractEnabled && !opportunitiesEnabled) {
    return {
      contractAwardsRequests: 0,
      opportunitiesRequests: 0,
      rationale: { reason: 'no_enabled_endpoints' }
    };
  }

  if (input.singlePassPerEndpoint) {
    const contractAwardsRequests = Math.min(
      SAM_CONTRACT_AWARDS_REQUESTS_PER_RUN_LIMIT,
      Math.min(Math.ceil(totalRequests / 2), totalRequests)
    );
    const opportunitiesRequests = Math.min(
      SAM_OPPORTUNITIES_REQUESTS_PER_RUN_LIMIT,
      Math.max(0, totalRequests - contractAwardsRequests)
    );
    return {
      contractAwardsRequests,
      opportunitiesRequests,
      rationale: {
        reason: 'hard_split_single_pass',
        totalRequests
      }
    };
  }
  ...
  if (!input.allocationEnabled) {
    return {
      contractAwardsRequests: Math.min(
        SAM_CONTRACT_AWARDS_REQUESTS_PER_RUN_LIMIT,
        Math.min(Math.ceil(totalRequests / 2), totalRequests)
      ),
      opportunitiesRequests: Math.min(
        SAM_OPPORTUNITIES_REQUESTS_PER_RUN_LIMIT,
        Math.max(0, totalRequests - Math.min(Math.ceil(totalRequests / 2), totalRequests))
      ),
      rationale: {
        reason: 'hard_split_allocation_disabled',
        totalRequests
      }
    };
  }
  ...
  return {
    ...
    opportunitiesRequests,
    rationale: {
      reason: 'hard_split_allocation',
      totalRequests,
      contractAwardsBudgetCap: SAM_CONTRACT_AWARDS_REQUESTS_PER_RUN_LIMIT,
      opportunitiesBudgetCap: SAM_OPPORTUNITIES_REQUESTS_PER_RUN_LIMIT
    }
  };
}

async function estimateSamEndpointBacklogs(
...
): Promise<SamEndpointBacklogSignals> {
  const signals: SamEndpointBacklogSignals = {
    contractAwardsCandidates: 0,
    opportunitiesCandidates: 0
  };
  const requestBudget = Math.max(1, Math.trunc(input.requestBudget));
  const normalizedTargetScopes = normalizeProgramScopeOrder(input.targetScopes);
  const scopeCoverageRatio = normalizedTargetScopes.length
    ? normalizedTargetScopes.length / TARGET_SAM_PROGRAM_SCOPES.length
    : 1;
  const cappedCoverageRatio = clampNumber(scopeCoverageRatio, 0.2, 1);

  if (input.runContractAwards) {
    const contractCandidateCap = Math.max(
      requestBudget,
      Math.trunc(requestBudget * SAM_CONTRACT_AWARDS_CANDIDATE_MULTIPLIER * cappedCoverageRatio)
    );
    const { count, error } = await supabase
      .from('artemis_contract_actions')
      .select('contract_id', { count: 'planned', head: true })
      .is('solicitation_id', null);
    if (error) throw error;
    signals.contractAwardsCandidates = clampBacklogCount(count, contractCandidateCap);
  }

  if (input.runOpportunities) {
    const opportunitiesCandidateCap = Math.max(
      requestBudget,
      Math.trunc(
        requestBudget *
          SAM_OPPORTUNITIES_SOLICITATION_SCAN_MULTIPLIER *
          100 *
          cappedCoverageRatio
      )
    );
    ...
    if (error) throw error;
    signals.opportunitiesCandidates = clampBacklogCount(
      count,
      opportunitiesCandidateCap
    );
  }
...
function clampBacklogCount(count: number | null, cap: number) {
  if (typeof count !== 'number' || !Number.isFinite(count) || count <= 0) return 0;
  return Math.max(0, Math.min(Math.trunc(count), Math.max(1, Math.trunc(cap))));
}
...
async function reconcileStaleIngestionRuns(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  input: {
    jobName: string;
    timeoutMs: number;
    excludeRunIds?: Array<string>;
  }
) {
  const staleBeforeIso = new Date(Date.now() - Math.max(60_000, Math.trunc(input.timeoutMs))).toISOString();
  const excluded = new Set(
    (input.excludeRunIds || []).filter(
  ...
  const { data, error } = await supabase
    ...
    .limit(STALE_INGESTION_RUN_REPAIR_LIMIT);
  if (error) throw error;

  const staleIds = (data || [])
    .map((row) => {
      if (typeof row.id === 'string' && row.id.trim().length > 0) return row.id.trim();
      if (typeof row.id === 'number' && Number.isFinite(row.id)) return String(Math.trunc(row.id));
      return null;
    })
    .filter((value): value is string => typeof value === 'string' && value.length > 0 && !excluded.has(value));
  if (!staleIds.length) return 0;

  const nowIso = new Date().toISOString();
  for (const chunk of chunkArray(staleIds, 50)) {
    const { error: updateError } = await supabase
      .from('ingestion_runs')
      .update({
        ended_at: nowIso,
        success: false,
        error: 'stale_run_reconciled_timeout'
      })
      .in('id', chunk)
      .is('ended_at', null);
...
serve(async (req) => {
  ...
  const stats: Record<string, unknown> = {
    ...
    samOpportunitiesDataServicesRequestsAttempted: 0,
    samOpportunitiesDataServicesRequestsGranted: 0,
    samEntityRequestsAttempted: 0,
    samEntityRequestsGranted: 0,
    samRunRequestCapRequested: 0,
    samRunRequestCap: 0,
    samRunRequestsRemaining: 0,
    samEndpointBudgetCaps: {
      contractAwards: SAM_CONTRACT_AWARDS_REQUESTS_PER_RUN_LIMIT,
      opportunities: SAM_OPPORTUNITIES_REQUESTS_PER_RUN_LIMIT,
      opportunitiesDataServices: SAM_OPPORTUNITIES_DATA_SERVICES_REQUESTS_PER_RUN_LIMIT
    },
    samRequestAllocationEnabled: DEFAULT_SAM_REQUEST_ALLOCATION_ENABLED,
    samRequestAllocationPlans: [] as Array<Record<string, unknown>>,
    samBacklogSignals: { contractAwardsCandidates: 0, opportunitiesCandidates: 0 } as SamEndpointBacklogSignals,
    samProbeMaxBudgetShare: DEFAULT_SAM_PROBE_MAX_BUDGET_SHARE,
    samProbeMinPostBudget: DEFAULT_SAM_PROBE_MIN_POST_BUDGET,
    samProbeBudgetShare: 0,
    samProbeSkippedByBudgetGuard: false,
    samOpportunitiesApiWeightWhenDataServices: DEFAULT_SAM_OPPORTUNITIES_API_WEIGHT_WHEN_DATA_SERVICES,
    ...
    staleIngestionRunTimeoutMs: STALE_INGESTION_RUN_TIMEOUT_MS,
  ...
  try {
    try {
      const staleClosed = await reconcileStaleIngestionRuns(supabase, {
        jobName: RUN_NAME,
        timeoutMs: STALE_INGESTION_RUN_TIMEOUT_MS,
        excludeRunIds: [runId]
      });
      stats.staleIngestionRunsClosed = staleClosed;
    } catch (error) {
      (stats.errors as Array<{ step: string; error: string }>).push({
        step: 'stale_run_reconcile',
    ...
    const probeBothEndpointsFirst = await readBooleanSetting(supabase, SETTING_SAM_PROBE_BOTH_ENDPOINTS_FIRST, false);
    const configuredSamSinglePassPerEndpoint = await readBooleanSetting(
      supabase,
      SETTING_SAM_SINGLE_PASS_PER_ENDPOINT,
      true
    );
    const hasBodySamEntitySyncFlag = Object.prototype.hasOwnProperty.call(body, 'samEntitySyncEnabled');
    const bodySamEntitySyncRaw = hasBodySamEntitySyncFlag ? readBooleanValue(body.samEntitySyncEnabled) : null;
    const hasBodySamExtractFlag = Object.prototype.hasOwnProperty.call(body, 'samContractAwardsExtractEnabled');
    const bodySamExtractRaw = hasBodySamExtractFlag ? readBooleanValue(body.samContractAwardsExtractEnabled) : null;
    ...
    const stopReasonIsGuardrail = (reason: string | null, includeNoDataAndNoCandidates: boolean) => {
      ...
      if (reason.startsWith('sam_http_error_')) {
        const status = Number.parseInt(reason.slice('sam_http_error_'.length), 10);
    ...
    if ((runSamContractAwards || runSamOpportunities) && !samApiKey) {
      ...
      const samQueryPolicy: SamQueryPolicy = {
        ...
        invalidRequestCooldownDays: Math.max(
          1,
          Math.min(
            90,
            Math.trunc(
              await readNumberSetting(
                supabase,
                SETTING_SAM_QUERY_COOLDOWN_DAYS_INVALID_REQUEST,
                DEFAULT_SAM_QUERY_COOLDOWN_DAYS_INVALID_REQUEST
              )
            )
          )
        ),
      ...
      const samOpportunitiesDataServicesMaxFileBytes = runSamOpportunities
        ...
        : DEFAULT_SAM_OPPORTUNITIES_DATA_SERVICES_MAX_FILE_BYTES;
      const samRequestAllocationEnabled = runSamContractAwards || runSamOpportunities
        ? await readBooleanSetting(
            supabase,
            SETTING_SAM_REQUEST_ALLOCATION_ENABLED,
            DEFAULT_SAM_REQUEST_ALLOCATION_ENABLED
          )
        : DEFAULT_SAM_REQUEST_ALLOCATION_ENABLED;
      const samProbeMaxBudgetShare = runSamContractAwards || runSamOpportunities
        ? Math.max(
            0,
            Math.min(
              1,
              await readNumberSetting(
                supabase,
                SETTING_SAM_PROBE_MAX_BUDGET_SHARE,
                DEFAULT_SAM_PROBE_MAX_BUDGET_SHARE
              )
            )
          )
        : DEFAULT_SAM_PROBE_MAX_BUDGET_SHARE;
      const samProbeMinPostBudget = runSamContractAwards || runSamOpportunities
        ? Math.max(
            0,
            Math.min(
              50,
              Math.trunc(
                await readNumberSetting(
                  supabase,
                  SETTING_SAM_PROBE_MIN_POST_BUDGET,
                  DEFAULT_SAM_PROBE_MIN_POST_BUDGET
                )
              )
            )
          )
        : DEFAULT_SAM_PROBE_MIN_POST_BUDGET;
      const samOpportunitiesApiWeightWhenDataServices = runSamOpportunities
        ? Math.max(
            0.1,
            Math.min(
              1.5,
              await readNumberSetting(
                supabase,
                SETTING_SAM_OPPORTUNITIES_API_WEIGHT_WHEN_DATA_SERVICES,
                DEFAULT_SAM_OPPORTUNITIES_API_WEIGHT_WHEN_DATA_SERVICES
              )
            )
          )
        : DEFAULT_SAM_OPPORTUNITIES_API_WEIGHT_WHEN_DATA_SERVICES;
      const configuredSamEntitySyncEnabled = runSamContractAwards || runSamOpportunities
        ? await readBooleanSetting(supabase, SETTING_SAM_ENTITY_SYNC_ENABLED, DEFAULT_SAM_ENTITY_SYNC_ENABLED)
        : false;
      const samEntitySyncEnabled =
        bodySamEntitySyncRaw === null ? configuredSamEntitySyncEnabled : bodySamEntitySyncRaw;
      ...
      const configuredSamContractAwardsExtractEnabled = runSamContractAwards
        ? await readBooleanSetting(
            supabase,
        ...
        : false;
      const samContractAwardsExtractEnabled =
        bodySamExtractRaw === null ? configuredSamContractAwardsExtractEnabled : bodySamExtractRaw;
      ...
      stats.samRequestAllocationEnabled = samRequestAllocationEnabled;
      stats.samProbeMaxBudgetShare = samProbeMaxBudgetShare;
      stats.samProbeMinPostBudget = samProbeMinPostBudget;
      stats.samOpportunitiesApiWeightWhenDataServices = samOpportunitiesApiWeightWhenDataServices;
      ...
      const applyOpportunitiesDataServicesResult = (result: SamOpportunitiesDataServicesSyncResult) => {
        ...
        stats.samRequestsAttempted = Number(stats.samRequestsAttempted || 0) + result.samRequestsAttempted;
        stats.samRequestsGranted = Number(stats.samRequestsGranted || 0) + result.samRequestsGranted;
        stats.samOpportunitiesRequestsAttempted =
          Number(stats.samOpportunitiesRequestsAttempted || 0) + result.samRequestsAttempted;
        stats.samOpportunitiesRequestsGranted =
          Number(stats.samOpportunitiesRequestsGranted || 0) + result.samRequestsGranted;
        stats.samOpportunitiesDataServicesRequestsAttempted =
          Number(stats.samOpportunitiesDataServicesRequestsAttempted || 0) + result.samRequestsAttempted;
        stats.samOpportunitiesDataServicesRequestsGranted =
          Number(stats.samOpportunitiesDataServicesRequestsGranted || 0) + result.samRequestsGranted;
        ...
        if (result.samQuota) stats.samQuota = result.samQuota;
        if (result.samQuotaBlocked) stats.samQuotaBlocked = true;
        if (result.stopReason && shouldStopSamRunForEndpoint('opportunities', result.stopReason)) {
          samOpportunitiesStopReason = result.stopReason;
          recordSamRunStop('opportunities', result.stopReason, {
            phase: 'data-services',
            details: {
      ...
      const runContractAwardsPass = async (
        ...
        const contractAwardsBudgetRemaining = Math.max(
          0,
          SAM_CONTRACT_AWARDS_REQUESTS_PER_RUN_LIMIT - Number(stats.samAwardsRequestsGranted || 0)
        );
        const effectiveMaxRequests = Math.max(0, Math.min(maxRequests, contractAwardsBudgetRemaining));
        if (!runSamContractAwards || !contractAwardsApiUrl) {
          logSamEndpointDecision('contract-awards', options.phase, 'skip', 'endpoint_disabled', {
            configured: false,
        ...
        if (effectiveMaxRequests < 1) {
          if (contractAwardsBudgetRemaining < 1) {
            logSamEndpointDecision('contract-awards', options.phase, 'skip', 'endpoint_budget_exhausted', {
              targetScopes: normalizedTargetScopes,
              remainingSamRequestsBefore: remainingSamRequests
            });
            return null;
      ...
      const runOpportunitiesPass = async (
        ...
        const normalizedTargetScopes = normalizeProgramScopeOrder(options.targetScopes || TARGET_SAM_PROGRAM_SCOPES);
        const opportunitiesBudgetRemaining = Math.max(
          0,
          SAM_OPPORTUNITIES_REQUESTS_PER_RUN_LIMIT - Number(stats.samOpportunitiesRequestsGranted || 0)
        );
        const effectiveMaxRequests = Math.max(0, Math.min(maxRequests, opportunitiesBudgetRemaining));
        ...
        let dataServicesRequestsGrantedThisPass = 0;
        if (samOpportunitiesDataServicesEnabled && !samOpportunitiesDataServicesSynced && effectiveMaxRequests > 0) {
          const dataServicesResult = await syncSamOpportunitiesDataServicesSnapshots(supabase, {
            ...
            maxRequests: Math.min(effectiveMaxRequests, SAM_OPPORTUNITIES_DATA_SERVICES_REQUESTS_PER_RUN_LIMIT),
          ...
          dataServicesRequestsGrantedThisPass = dataServicesResult.samRequestsGranted;
          ...
          pushSamStep('sam_opportunities_data_services_sync', {
            ...
            samRequestsAttempted: dataServicesResult.samRequestsAttempted,
            samRequestsGranted: dataServicesResult.samRequestsGranted,
        ...
        if (samRunStopReason || samGuardrailTriggered) {
          logSamEndpointDecision('opportunities', options.phase, 'skip', 'post_data_services_guardrail', {
            targetScopes: normalizedTargetScopes,
            remainingSamRequestsBefore: remainingSamRequests,
            stoppedByEndpoint: samRunStopByEndpoint,
            stopReason: samRunStopReason || stringOrNull(stats.samRequestStopReason)
          });
          return null;
        }
        const remainingApiRequests = Math.max(
          0,
          effectiveMaxRequests - dataServicesRequestsGrantedThisPass
        );
        if (effectiveMaxRequests < 1) {
          if (opportunitiesBudgetRemaining < 1) {
            logSamEndpointDecision('opportunities', options.phase, 'skip', 'endpoint_budget_exhausted', {
              targetScopes: normalizedTargetScopes,
              remainingSamRequestsBefore: remainingSamRequests
            });
            return null;
        ...
        if (remainingApiRequests < 1) {
          logSamEndpointDecision('opportunities', options.phase, 'skip', 'api_budget_consumed_by_data_services', {
            targetScopes: normalizedTargetScopes,
            remainingSamRequestsBefore: remainingSamRequests
          });
          return null;
        ...
        logSamEndpointDecision('opportunities', options.phase, 'start', null, {
          maxRequests: remainingApiRequests,
          targetScopes: normalizedTargetScopes,
          remainingSamRequestsBefore: remainingSamRequests,
      ...
      const initialProbeBudgetShare = requiredProbeRequests > 0 && remainingSamRequests > 0
        ? requiredProbeRequests / remainingSamRequests
        : 0;
      const shouldSkipProbeForBudget =
        shouldProbeBothEndpoints &&
        (
          remainingSamRequests < requiredProbeRequests ||
          initialProbeBudgetShare > samProbeMaxBudgetShare ||
          remainingSamRequests - requiredProbeRequests < samProbeMinPostBudget
        );
      stats.samProbeBudgetShare = initialProbeBudgetShare;
      stats.samProbeSkippedByBudgetGuard = shouldSkipProbeForBudget;
      if (shouldSkipProbeForBudget) {
        pushSamStep('sam_probe_skipped_budget_guard', {
          remainingSamRequests,
          requiredProbeRequests,
          probeBudgetShare: initialProbeBudgetShare,
          maxBudgetShare: samProbeMaxBudgetShare,
          minPostBudget: samProbeMinPostBudget
        });
      }

      let samBacklogSignals: SamEndpointBacklogSignals = {
        contractAwardsCandidates: 0,
        opportunitiesCandidates: 0
      };
      try {
        if (samRequestAllocationEnabled && !singlePassPerEndpoint && (runSamContractAwards || runSamOpportunities)) {
          samBacklogSignals = await estimateSamEndpointBacklogs(supabase, {
            runContractAwards: runSamContractAwards,
            runOpportunities: runSamOpportunities,
            targetScopes: TARGET_SAM_PROGRAM_SCOPES,
            requestBudget: Math.max(1, remainingSamRequests)
          });
        }
      } catch (error) {
        const message = stringifyError(error);
        (stats.errors as Array<{ step: string; error: string }>).push({
          step: 'sam_backlog_signal_estimate',
          error: message
        });
        pushSamStep('sam_backlog_signal_estimate_error', { error: message });
      }
      stats.samBacklogSignals = samBacklogSignals;
      pushSamStep('sam_backlog_signal_estimate', samBacklogSignals);
      ...
      const runBalancedSamPass = async (phase: string, requestedTotal: number) => {
        if (samRunStopReason || samGuardrailTriggered) return;
        const allocationPlanRaw = planSamRequestAllocation({
          totalRequests: requestedTotal,
          runContractAwards: runSamContractAwards,
          runOpportunities: runSamOpportunities,
          singlePassPerEndpoint,
          allocationEnabled: samRequestAllocationEnabled,
          opportunitiesDataServicesEnabled: samOpportunitiesDataServicesEnabled,
          opportunitiesApiDeltaOnly: samOpportunitiesApiDeltaOnly,
          opportunitiesApiWeightWhenDataServices: samOpportunitiesApiWeightWhenDataServices,
          contractAwardsBacklog: samBacklogSignals.contractAwardsCandidates,
          opportunitiesBacklog: samBacklogSignals.opportunitiesCandidates,
          contractAwardsYield: rowsPerRequest(
            Number(stats.samAwardsRowsUpserted || 0),
            Number(stats.samAwardsRequestsGranted || 0)
          ),
          opportunitiesYield: rowsPerRequest(
            Number(stats.samNoticesUpserted || 0),
            Number(stats.samOpportunitiesRequestsGranted || 0)
          )
        });
        const contractAwardsBudgetRemaining = Math.max(
          0,
        ...
        const allocationPlan = {
          ...allocationPlanRaw,
          contractAwardsRequests: Math.min(
            allocationPlanRaw.contractAwardsRequests,
            contractAwardsBudgetRemaining
          ),
          opportunitiesRequests: Math.min(
            allocationPlanRaw.opportunitiesRequests,
            opportunitiesBudgetRemaining
          ),
          rationale: {
            ...allocationPlanRaw.rationale,
            contractAwardsBudgetRemaining,
            opportunitiesBudgetRemaining
          }
        };
        (stats.samRequestAllocationPlans as Array<Record<string, unknown>>).push({
          phase,
          requestedTotal,
          ...allocationPlan
        });
        pushSamStep('sam_request_allocation_plan', {
          phase,
          requestedTotal,
          ...allocationPlan
        });

        if (allocationPlan.contractAwardsRequests > 0) {
          await runContractAwardsPass(allocationPlan.contractAwardsRequests, { phase });
        }

        const shouldForceOpportunitiesSync =
          runSamOpportunities &&
          samOpportunitiesDataServicesEnabled &&
          !samOpportunitiesDataServicesSynced;
        if (
          !samRunStopReason &&
          !samGuardrailTriggered &&
          (allocationPlan.opportunitiesRequests > 0 || shouldForceOpportunitiesSync)
        ) {
          await runOpportunitiesPass(
            allocationPlan.opportunitiesRequests,
            { phase },
            Array.from(targetedSolicitationIds)
          );
      ...
      if (shouldProbeBothEndpoints && !shouldSkipProbeForBudget) {
        await runPerScopeInitialPass('probe', 1);
      } else if (shouldProbeBothEndpoints) {
        logSamEndpointDecision('contract-awards', 'probe', 'skip', 'probe_budget_guard', {
          remainingSamRequests,
          requiredProbeRequests,
          probeBudgetShare: initialProbeBudgetShare,
          maxBudgetShare: samProbeMaxBudgetShare,
        ...
        logSamEndpointDecision('opportunities', 'probe', 'skip', 'probe_budget_guard', {
          remainingSamRequests,
          requiredProbeRequests,
          probeBudgetShare: initialProbeBudgetShare,
          maxBudgetShare: samProbeMaxBudgetShare,
      ...
      if (!samRunStopReason && !samGuardrailTriggered && remainingSamRequests > 0) {
        if (singlePassPerEndpoint) {
          await runBalancedSamPass('single', remainingSamRequests);
        } else {
          const beforePassOne = remainingSamRequests;
          await runBalancedSamPass('balanced:1', remainingSamRequests);
          const consumedInPassOne = Math.max(0, beforePassOne - remainingSamRequests);

          if (!samRunStopReason && !samGuardrailTriggered && remainingSamRequests > 0 && consumedInPassOne > 0) {
            const beforePassTwo = remainingSamRequests;
            await runBalancedSamPass('balanced:2', remainingSamRequests);
            const consumedInPassTwo = Math.max(0, beforePassTwo - remainingSamRequests);

            if (!samRunStopReason && !samGuardrailTriggered && remainingSamRequests > 0 && consumedInPassTwo < 1) {
              const awardsYield = rowsPerRequest(
                Number(stats.samAwardsRowsUpserted || 0),
                Number(stats.samAwardsRequestsGranted || 0)
              );
              const opportunitiesYield = rowsPerRequest(
                Number(stats.samNoticesUpserted || 0),
                Number(stats.samOpportunitiesRequestsGranted || 0)
              );
              const prioritizeContractAwards =
                awardsYield > opportunitiesYield ||
                (awardsYield === opportunitiesYield &&
                  samBacklogSignals.contractAwardsCandidates >= samBacklogSignals.opportunitiesCandidates);
              const winner = prioritizeContractAwards ? 'contract-awards' : 'opportunities';
              pushSamStep('sam_request_allocation_fallback', {
              ...
              if (winner === 'contract-awards' && runSamContractAwards) {
                await runContractAwardsPass(remainingSamRequests, { phase: 'balanced:fallback' });
              } else if (runSamOpportunities) {
                await runOpportunitiesPass(remainingSamRequests, { phase: 'balanced:fallback' });
              }
            }
          }
        }
    ...
    const samFailureStopReason =
      (finalSamStopReason && isSamEndpointFailureStopReason(finalSamStopReason) ? finalSamStopReason : null) ||
      (samContractAwardsStopReason && isSamEndpointFailureStopReason(samContractAwardsStopReason)
        ? samContractAwardsStopReason
        : null) ||
      (samOpportunitiesStopReason && isSamEndpointFailureStopReason(samOpportunitiesStopReason)
        ? samOpportunitiesStopReason
        : null);
    const samFailureEndpoint =
      (samRunStopByEndpoint && finalSamStopReason === samFailureStopReason ? samRunStopByEndpoint : null) ||
      (samContractAwardsStopReason && samContractAwardsStopReason === samFailureStopReason ? 'contract-awards' : null) ||
      (samOpportunitiesStopReason && samOpportunitiesStopReason === samFailureStopReason ? 'opportunities' : null);
    if (samFailureStopReason && !stats.samRequestStopReason) {
      stats.samRequestStopReason = samFailureStopReason;
    }
    if (samFailureStopReason && !finalSamStopReasons.includes(samFailureStopReason)) {
      finalSamStopReasons.push(samFailureStopReason);
    }
    const buildSamCheckpointState = (
      endpoint: 'contract-awards' | 'opportunities',
      endpointStopReason: string | null
    ) => {
      const ownFailureStopReason =
        endpointStopReason && isSamEndpointFailureStopReason(endpointStopReason) ? endpointStopReason : null;
      const inheritedFailureStopReason =
        !ownFailureStopReason &&
        samFailureStopReason &&
        samFailureEndpoint &&
        samFailureEndpoint !== endpoint
          ? samFailureStopReason
          : null;
      const failureStopReason = ownFailureStopReason || inheritedFailureStopReason;
      if (!failureStopReason) {
        return {
          status: 'complete' as const,
          lastError: null,
          failureStopReason: null,
          retryable: false
        };
      }

      if (inheritedFailureStopReason) {
        const upstreamFailureMessage = buildSamFailureMessage({
          endpoint: samFailureEndpoint,
          stopReason: inheritedFailureStopReason
        }).replace(/\.$/, '');
        return {
          status: 'error' as const,
          lastError: `${formatSamEndpointLabel(endpoint)} pass did not complete because ${upstreamFailureMessage}.`,
          failureStopReason,
          retryable: isRetryableSamStopReason(failureStopReason)
        };
      }

      return {
        status: 'error' as const,
        lastError: buildSamFailureMessage({
          endpoint,
    ...
    const samContractAwardsCheckpointState = buildSamCheckpointState(
      'contract-awards',
      samContractAwardsStopReason || null
    );
    const samOpportunitiesCheckpointState = buildSamCheckpointState('opportunities', samOpportunitiesStopReason || null);
    const samRunFailed = Boolean(samFailureStopReason);
    const samRunRetryable = isRetryableSamStopReason(samFailureStopReason);
    const samRunFailureMessage =
      samFailureStopReason && samRunFailed
        ? buildSamFailureMessage({
            endpoint: samFailureEndpoint,
            stopReason: samFailureStopReason
          })
        : null;
    stats.samRunFailed = samRunFailed;
    stats.samRunRetryable = samRunRetryable;
    stats.samRunFailureMessage = samRunFailureMessage;


    if (runSamContractAwards) {
      await updateCheckpoint(supabase, CHECKPOINT_SAM_CONTRACT_AWARDS, {
        sourceType: 'procurement',
        status: samContractAwardsCheckpointState.status,
        recordsIngested: Number(stats.samAwardsRowsUpserted || 0),
        endedAt: new Date().toISOString(),
        lastError: samContractAwardsCheckpointState.lastError,
        metadata: {
          ...checkpointMetadata,
          ...
          samRequestStopReason: stringOrNull(stats.samRequestStopReason) || finalSamStopReason || samFailureStopReason || null,
          samRunStopReason: samRunStopReason || finalSamStopReason || samFailureStopReason || null,
          samRunStoppedByEndpoint: samRunStopByEndpoint || samFailureEndpoint || null,
          ...
          failureStopReason: samContractAwardsCheckpointState.failureStopReason,
          retryableFailure: samContractAwardsCheckpointState.retryable,
          failureMessage: samContractAwardsCheckpointState.lastError,
    ...
    if (runSamOpportunities) {
      await updateCheckpoint(supabase, CHECKPOINT_OPPORTUNITIES, {
        sourceType: 'procurement',
        status: samOpportunitiesCheckpointState.status,
        recordsIngested: Number(stats.samNoticesUpserted || 0),
        endedAt: new Date().toISOString(),
        lastError: samOpportunitiesCheckpointState.lastError,
        metadata: {
          ...checkpointMetadata,
          ...
          failureStopReason: samOpportunitiesCheckpointState.failureStopReason,
          retryableFailure: samOpportunitiesCheckpointState.retryable,
          failureMessage: samOpportunitiesCheckpointState.lastError,
          probeBothEndpointsFirst: shouldProbeBothEndpoints,
          probeStopReasons: getSamProbeStopReasons()
        }
      });
    ...
    if (samRunFailureMessage) {
      const errors = stats.errors as Array<{ step: string; error: string }>;
      const existingFailure = errors.some((entry) => entry.step === 'sam_finalize' && entry.error === samRunFailureMessage);
      if (!existingFailure) errors.push({ step: 'sam_finalize', error: samRunFailureMessage });
      pushSamStep('sam_finalize_failure', {
        stopReason: samFailureStopReason,
        failedEndpoint: samFailureEndpoint,
        retryable: samRunRetryable,
    ...
    await endRunPhase('finalize_outputs', {
      ...
      success: !samRunFailed,
      samRunFailed,
      samRunRetryable,
      samRunFailureMessage
    });
    await beginRunPhase('finish_run');
    await endRunPhase('finish_run', samRunFailed ? { success: false, error: samRunFailureMessage } : { success: true });
    await persistRunProgress({ force: true, phase: null });

    if (samRunFailed) {
      const failureMessage = samRunFailureMessage || 'SAM endpoint failure';
      await finishIngestionRun(supabase, runId, false, stats, failureMessage);
      return jsonResponse(
        { ok: false, runId, error: failureMessage, elapsedMs: Date.now() - startedAt, stats },
        samRunRetryable ? 503 : 500
      );
    }
...
function normalizeSamLookupIdentifiers(piid: string | null, referencedIdvPiid: string | null) {
...
async function fetchProcurementAwards(
  ...
  return ((data || []) as ProcurementAwardRow[]).filter((row) => {
    ...
    return Boolean(stringOrNull(row.usaspending_award_id) || stringOrNull(readMetaString(metadata, 'piid')));
  });
}
...
function buildNormalizedContracts(rows: ProcurementAwardRow[]) {
  ...
  for (const row of rows) {
    ...
    const candidate: ContractRow = {
      ...
      subtier_code: stringOrNull(readMetaString(meta, 'subtierCode')),
...
async function backfillSolicitationsFromSamContractAwards(
  ...
  const candidates = await fetchContractAwardsLookupCandidates(
    ...
    Math.max(
      1,
      Math.min(
        SAM_CONTRACT_AWARDS_ACTION_SCAN_LIMIT_MAX,
        input.maxRequests * SAM_CONTRACT_AWARDS_CANDIDATE_MULTIPLIER
      )
    ),
    input.targetScopes,
  ...
  for (const candidate of candidates) {
    ...
    while (continuePagingCandidate) {
      ...
      const queryParams: Record<string, unknown> = {
        ...
        piidSubtierCode: candidate.piidSubtierCode || null,
      ...
      const sourceDocId = await insertSourceDocument(supabase, {
        ...
        raw: {
          ...
          piidSubtierCode: candidate.piidSubtierCode,
          includeDeletedStatus: input.includeDeletedStatus,
          includeSections: input.includeSections,
          offset: candidateOffset,
          queryFingerprint: queryGate.fingerprint,
...
function buildSamAwardExtractJobKey(input: {
  ...
  const hash = deterministicHash(
    stableJsonStringify({
      ...
      piidSubtierCode: input.candidate.piidSubtierCode || null,
      format: input.format,
      includeDeletedStatus: input.includeDeletedStatus,
...
function buildCandidateFromSamExtractJob(job: SamAwardExtractJobRow): ContractAwardsLookupCandidate | null {
  ...
  const payloadCandidate = safeRecord(payload.candidate);
  const piidSubtierCode = normalizeSamIdentifier(
    stringOrNull(payloadCandidate.piidSubtierCode) || stringOrNull(payloadCandidate.awardingSubTier)
  );
  return {
    contractId,
...
async function requestSamAwardExtractJob(
  ...
  if (input.candidate.piidSubtierCode) {
    url.searchParams.set('piidSubtierCode', input.candidate.piidSubtierCode);
  }
  ...
  const invalidParams = findUnexpectedSamQueryParams(url, SAM_CONTRACT_AWARDS_EXTRACT_ALLOWED_QUERY_PARAMS);
  if (invalidParams.length > 0) {
    const body = buildSamInvalidRequestPayload({
      endpoint: 'contract-awards-extract',
      unexpectedParams: invalidParams
    });
    result.stopReason = 'sam_http_error_400';
    const sourceDocId = await insertSourceDocument(supabase, {
      sourceKey: CHECKPOINT_SAM_CONTRACT_AWARDS,
      sourceType: 'procurement',
      url: sanitizeSamRequestUrl(url.toString()),
      title: `SAM contract awards extract request (${input.candidate.contractKey})`,
      summary: `SAM contract awards extract request rejected before fetch; invalid params ${invalidParams.join(', ')}.`,
      announcedTime: new Date().toISOString(),
      httpStatus: 400,
      contentType: 'application/json',
      raw: {
        ...
        piidSubtierCode: input.candidate.piidSubtierCode,
        format: input.format,
        includeDeletedStatus: input.includeDeletedStatus,
        includeSections: input.includeSections,
        body
...
async function backfillSolicitationsFromExistingSamAwardRows(
  ...
  for (const contractId of orderedContractIds) {
    ...
    const piidSubtierCode = normalizeSamIdentifier(
      stringOrNull(row.subtier_code) ||
        readMetaString(metadata, 'piidSubtierCode') ||
        readMetaString(metadata, 'piid_subtier_code') ||
        readMetaString(metadata, 'awardingSubtierCode') ||
        readMetaString(metadata, 'awarding_subtier_code') ||
        readMetaString(metadata, 'awardingSubTier') ||
        readMetaString(metadata, 'awarding_sub_tier')
    );
    candidates.push({
      contractId,
...
async function fetchContractAwardsLookupCandidates(
  ...
  const actionScanLimit = Math.max(1, Math.min(SAM_CONTRACT_AWARDS_ACTION_SCAN_LIMIT_MAX, limit));
  const [recentActionsResult, backlogActionsResult] = await Promise.all([
    supabase
      .from('artemis_contract_actions')
      .select('contract_id,updated_at')
      .is('solicitation_id', null)
      .order('updated_at', { ascending: false, nullsFirst: false })
      .limit(actionScanLimit),
    supabase
      .from('artemis_contract_actions')
      .select('contract_id,updated_at')
      .is('solicitation_id', null)
  ...
  if (recentActionsResult.error) throw recentActionsResult.error;
  if (backlogActionsResult.error) throw backlogActionsResult.error;

  const recentActionRows = (recentActionsResult.data || []) as Array<Record<string, unknown>>;
  const backlogActionRows = (backlogActionsResult.data || []) as Array<Record<string, unknown>>;
  const missingCountByContract = new Map<string, number>();

  const collectOrderedContracts = (rows: Array<Record<string, unknown>>) => {
    ...
    for (const row of rows) {
      ...
      seen.add(contractId);
      ordered.push(contractId);
    }
    return ordered;
  };

  const recentContractIds = collectOrderedContracts(recentActionRows);
  const backlogContractIds = collectOrderedContracts(backlogActionRows);

  const orderedContractIds: string[] = [];
  const orderedSeen = new Set<string>();
  const orderedLength = Math.max(recentContractIds.length, backlogContractIds.length);
  for (let index = 0; index < orderedLength; index += 1) {
    const recentContractId = recentContractIds[index];
    if (recentContractId && !orderedSeen.has(recentContractId)) {
      orderedSeen.add(recentContractId);
      orderedContractIds.push(recentContractId);
    }
    const backlogContractId = backlogContractIds[index];
    if (backlogContractId && !orderedSeen.has(backlogContractId)) {
      orderedSeen.add(backlogContractId);
      orderedContractIds.push(backlogContractId);
    }
  }
...
async function fetchContractsByIds(
  ...
  for (const chunk of chunkArray(normalizedContractIds, CONTRACT_LOOKUP_FETCH_CHUNK_SIZE)) {
    const { data, error } = await supabase
      .from('artemis_contracts')
      .select('id,contract_key,piid,referenced_idv_piid,subtier_code,mission_key,awardee_name,description,contract_type,metadata')
      .in('id', chunk);
    if (error) throw error;
...
async function fetchSamContractAwards(input: {
  ...
  const invalidParams = findUnexpectedSamQueryParams(url, SAM_CONTRACT_AWARDS_ALLOWED_QUERY_PARAMS);
  if (invalidParams.length > 0) {
    return {
      ok: false,
      status: 400,
      ...
      body: buildSamInvalidRequestPayload({
        endpoint: 'contract-awards',
        unexpectedParams: invalidParams
      })
    };
...
async function fetchSamOpportunities(input: {
  ...
  url.searchParams.set('limit', String(requestedLimit));
  url.searchParams.set('offset', String(requestedOffset));
  const invalidParams = findUnexpectedSamQueryParams(url, SAM_OPPORTUNITIES_ALLOWED_QUERY_PARAMS);
  if (invalidParams.length > 0) {
    return {
      ok: false,
      status: 400,
...
function readSamHttpErrorStatus(stopReason: string | null) {
  if (!stopReason || !stopReason.startsWith('sam_http_error_')) return null;
  const status = Number.parseInt(stopReason.slice('sam_http_error_'.length), 10);
  return Number.isFinite(status) ? status : null;
}

function readSamAuthErrorStatus(stopReason: string | null) {
  if (!stopReason || !stopReason.startsWith('sam_auth_error_')) return null;
  const status = Number.parseInt(stopReason.slice('sam_auth_error_'.length), 10);
  return Number.isFinite(status) ? status : null;
}

function isSamEndpointFailureStopReason(stopReason: string | null) {
  if (!stopReason) return false;
  if (stopReason === 'sam_quota_throttled') return true;
  if (stopReason === 'sam_http_404_not_found') return true;
  if (stopReason.startsWith('sam_auth_error_')) return true;
  return readSamHttpErrorStatus(stopReason) !== null;
}

function isRetryableSamStopReason(stopReason: string | null) {
  if (!stopReason) return false;
  if (stopReason === 'sam_quota_throttled') return true;
  const status = readSamHttpErrorStatus(stopReason);
  return status === 429 || (status !== null && status >= 500);
}

function formatSamEndpointLabel(endpoint: 'contract-awards' | 'opportunities' | null) {
  if (endpoint === 'contract-awards') return 'SAM contract-awards';
  if (endpoint === 'opportunities') return 'SAM opportunities';
  return 'SAM';
}

function buildSamFailureMessage(input: {
  endpoint: 'contract-awards' | 'opportunities' | null;
  stopReason: string;
}) {
  const label = formatSamEndpointLabel(input.endpoint);
  const httpStatus = readSamHttpErrorStatus(input.stopReason);
  if (httpStatus !== null) {
    const retryablePrefix = httpStatus === 429 || httpStatus >= 500 ? 'retryable ' : '';
    return `${label} request failed with ${retryablePrefix}HTTP ${httpStatus} from SAM.gov.`;
  }

  const authStatus = readSamAuthErrorStatus(input.stopReason);
  if (authStatus !== null) {
    return `${label} request failed with SAM.gov auth error HTTP ${authStatus}.`;
  }
  if (input.stopReason === 'sam_quota_throttled') {
    return `${label} request was throttled by SAM.gov.`;
  }
  if (input.stopReason === 'sam_http_404_not_found') {
    return `${label} request returned HTTP 404 from SAM.gov.`;
  }
  return `${label} failed with ${input.stopReason}.`;
}

function findUnexpectedSamQueryParams(url: URL, allowedParams: readonly string[]) {
  const allowed = new Set(allowedParams);
  const unexpected = new Set<string>();
  for (const key of url.searchParams.keys()) {
    if (!allowed.has(key)) unexpected.add(key);
  }
  return [...unexpected.values()].sort();
}

function buildSamInvalidRequestPayload(input: {
  endpoint: 'contract-awards' | 'opportunities' | 'contract-awards-extract';
  unexpectedParams: string[];
}) {
  return {
    error: 'sam_invalid_request_parameters',
    message: `Unexpected SAM query parameter(s): ${input.unexpectedParams.join(', ')}`,
    endpoint: input.endpoint,
    unexpectedParams: input.unexpectedParams
  };
}
...
function isSamInvalidRequestStatus(status: number) {
  return status >= 400 && status < 500 && status !== 401 && status !== 403 && status !== 404 && status !== 429;
}
...
async function recordSamQueryExecutionOutcome(
  ...
  } else if (isSamInvalidRequestStatus(status)) {
    consecutiveFailures = input.gate.existingFailures;
    nextRetryAt = null;
    cooldownUntil = new Date(
      Date.now() + Math.max(1, Math.trunc(input.policy.invalidRequestCooldownDays)) * MILLISECONDS_PER_DAY
    ).toISOString();
...
function extractOpportunityNotices(payload: unknown, solicitationId: string): OpportunityNoticeRow[] {
  ...
  const nowIso = new Date().toISOString();
  const notices: OpportunityNoticeRow[] = [];
...
async function syncSamOpportunitiesDataServicesSnapshots(
  ...
  if (Math.max(0, Math.trunc(input.maxRequests)) < 1) {
    result.skipped = true;
    result.skippedReason = 'max_requests_lt_1';
    return result;
  }
  ...
  for (const source of configuredSources) {
    if (result.samRequestsGranted >= Math.max(0, Math.trunc(input.maxRequests))) {
      break;
    }
  ...
  await updateCheckpoint(supabase, CHECKPOINT_OPPORTUNITIES_DATA_SERVICES, {
    ...
    metadata: {
      ...
      samRequestsAttempted: result.samRequestsAttempted,
      samRequestsGranted: result.samRequestsGranted,
      samQuotaBlocked: result.samQuotaBlocked,
      stopReason: result.stopReason,
