...
- 2026-04-08: AR trajectory `v3` data and product roadmap planning is in progress for `Web`, `iOS`, and `Android`.
  ...
  - Current implementation slice: telemetry completeness reporting, `rocket_family` repair instrumentation, coverage-policy enforcement for family readiness, a repo-level ingest admission template plus registry, seeded provider-level admission reviews for SpaceX, Blue Origin, and Rocket Lab, and admin-facing SpaceX adapter-health reporting are now in place.
