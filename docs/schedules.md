...
- Notifications cadence: Supabase `pg_cron` calls `notifications-dispatch` every 2 minutes and `notifications-send` every minute to queue native mobile push notifications.
...
- Trajectory constraints ingest: managed scheduler runs `trajectory-constraints-ingest` hourly (staggered) to fetch LL2 landings for top eligible launches, ensure `ll2_landings`, refresh `ll2_launch_landings`, and conditionally merge `launch_trajectory_constraints` (`constraint_type='landing'`).
